"""Offline multi-core Quagmire key cracker for the Strahd cipher.

Everything is embedded: the cipher and a large list of 13-letter keys (Curse
of Strahd terms plus every 13-letter English dictionary word). The ONLY
external file needed is english_quadgrams.txt in the same folder:
  https://raw.githubusercontent.com/jameslyons/python_cryptanalysis/master/quadgrams.txt

Uses all CPU cores. Console output only.
  python3 strahd_offline.py            crack: test every embedded key, ranked
  python3 strahd_offline.py MYKEYWORD  test one 13-letter keyword
"""
...
def init_worker():
    ...
    global _QUAD, _FLOOR, _LABELS
    _QUAD, _FLOOR = load_quadgrams(QUADGRAM_FILE)
    _LABELS = [ord(c) - 65 for c in CIPHER]


def quick_word(keyword):
    """Worker task: rough score for one keyword via a capped alphabet climb."""
    key = [ord(c) - 65 for c in keyword]
    return capped_climb(_LABELS, key, _QUAD, _FLOOR, random.Random(0)), keyword
...
def verdict(score):
    """Returns a short verdict line for a score."""
    if score > -3500:
        return ">>> PROMISING - read the text above, likely the key."
    return ">>> nothing cracked (near random); key not in this set."


def main():
    """Tests one keyword (arg) or every embedded keyword, on all cores."""
    labels = [ord(c) - 65 for c in CIPHER]

    if len(sys.argv) > 1:
        keyword = sys.argv[1].strip().upper()
        log_probs, floor = load_quadgrams(QUADGRAM_FILE)
        score, plaintext = best_for_key(labels, keyword, log_probs, floor, 15)
        print(f"{keyword}: score={score:.0f}  IoC={index_of_coincidence(plaintext):.4f}")
        print(plaintext)
        print(verdict(score))
        return

    words = sorted({w for w in WORDS if len(w) == KEY_LENGTH and w.isalpha()})
    cores = os.cpu_count() or 4
    print(f"Testing {len(words)} keys on {cores} cores. "
          f"Correct ~ -2800 / IoC 0.066; wrong ~ -4800.")

    start = time.time()
    scored = []
    with ProcessPoolExecutor(max_workers=cores, initializer=init_worker) as pool:
        for i, result in enumerate(pool.map(quick_word, words, chunksize=20), 1):
            scored.append(result)
            if i % 2000 == 0:
                rate = i / (time.time() - start)
                print(f"  {i}/{len(words)}  best {max(scored)[1]} "
                      f"({max(scored)[0]:.0f})  [{rate:.0f}/s]")
    scored.sort(reverse=True)
    print(f"Quick pass {time.time() - start:.0f}s. Refining top {TOP_TO_REFINE}...")

    log_probs, floor = load_quadgrams(QUADGRAM_FILE)
    results = []
    for _, keyword in scored[:TOP_TO_REFINE]:
        score, plaintext = best_for_key(labels, keyword, log_probs, floor, REFINE_RESTARTS)
        results.append((score, keyword, plaintext))
    results.sort(reverse=True)

    print("\nTOP 10:")
    for score, keyword, plaintext in results[:10]:
        print(f"  {keyword}  score={score:.0f}  IoC={index_of_coincidence(plaintext):.4f}")
    best = results[0]
    print(f"\nBEST: {best[1]}  score={best[0]:.0f}  IoC={index_of_coincidence(best[2]):.4f}")
    print(best[2])
    print(verdict(best[0]))
